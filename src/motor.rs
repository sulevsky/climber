...
pub struct Motors<'d, CHL: GeneralInstance4Channel, CHR: GeneralInstance4Channel> {
    left_direction: Output<'d>,
    left_pwm: SimplePwmChannel<'d, CHL>,
    right_direction: Output<'d>,
...
impl<'d, CHL: GeneralInstance4Channel, CHR: GeneralInstance4Channel> Motors<'d, CHL, CHR> {
    ...
    pub fn update(&mut self, motors_power: MotorsPower) {
        info!(
            "l/r {}/{} - motor update",
            motors_power.left, motors_power.right
        );
        if motors_power.left == 0 {
            self.left_direction.set_low();
            ...
            info!(
                "V< {} of {}-{} init l/r {}/{}",
                fraction,
                CALIBRATED_PWM_VALUE_BACKWARD.start,
                CALIBRATED_PWM_VALUE_BACKWARD.end,
                motors_power.left,
                motors_power.right
            );
            self.left_pwm
            ...
            info!(
                "^< {} of {}-{} init l/r {}/{}",
                fraction,
                CALIBRATED_PWM_VALUE_FORWARD.start,
                CALIBRATED_PWM_VALUE_FORWARD.end,
                motors_power.left,
                motors_power.right
            );
            self.left_pwm
